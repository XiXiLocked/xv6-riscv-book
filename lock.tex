...
重要的是，\indexcode{acquire} 必须严格在设置 \lstinline{lk->locked} \lineref{kernel/spinlock.c:/__atomic_exchange_n/} 之前调用 \lstinline{push_off}。如果顺序相反，就会存在一个短暂的窗口期，此时锁被持有但中断是使能的，一个时机不当的中断会导致系统死锁。类似地，重要的是 \indexcode{release} 必须在释放锁 \lineref{kernel/spinlock.c:/sync_lock_release/} 之后才调用 \indexcode{pop_off}。
